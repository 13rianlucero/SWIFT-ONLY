...
class ViewController: UIViewController , UIPickerViewDataSource, UIPickerViewDelegate{
    ...
    func checkWin(){
        ...
        if counter == 3{ // winning
            ...
            stepper.maximumValue = Double(currentCash)
            cashLabel.text = "\((currentCash + 200) + (cashToRisk * 2 ))"
            userIndicatorlabel.text = "YOU WON \(200 + cashToRisk * 2)$"
            vm.updateScore(cash: (currentCash + 200) + (cashToRisk * 2 ))
        } else { // losing
            userIndicatorlabel.text = "TRY AGAIN"
            cashLabel.text = "\((currentCash - cashToRisk))"
            vm.updateScore(cash: (currentCash - cashToRisk))
        }
